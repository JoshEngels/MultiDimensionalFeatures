...
for dataset_num in range(1, 5):
    ...
    def pick_fn(dataset_num):
        ...
        if dataset_num == 4:
            x = torch.randn([batch_size, n])
            x = torch.tensordot((3*x).type(torch.int32).type(x.dtype), randmat, dims=1)
        return n, x
    ...
    def get_parameters(x, epsilon=0.1):
        ...
        a = torch.randn([n], requires_grad=True)  # Random initialization, requires_grad=True to track gradients
        b = torch.zeros([], requires_grad=True)  # Random initialization, requires_grad=True to track gradients
    ...
    def optimize_mutual_info(xy, split, n, bound, angular_res):  # xy: n, d
        ...
        for i in range(angular_res):
            ...
            rotation = torch.cos(angle).type(xy.dtype)*torch.eye(2, dtype=xy.dtype) + torch.sin(angle).type(xy.dtype)*torch.tensor([[0, -1], [1, 0]], dtype=xy.dtype)
    ...
    if dataset_num == 4:
        height = 700
        xpos = 0.4
    axs[1].text(xpos, height, '$M_\\epsilon(\\mathbf{f})=' + str(round(float(P), 4)) + '$', color='red')
    ...
    axs[0].axline(-b + eps*normalized_a[:2], slope=-a[0]/a[1], color="red", linestyle=(0, (5, 5)))
    axs[0].axline(-b - eps*normalized_a[:2], slope=-a[0]/a[1], color="red", linestyle=(0, (5, 5)))
    ...
    n = 20
    bound = 3
    mutual_infos, max_ind, net_transform = optimize_mutual_info(xy, 1, n, bound, angular_res)
    ...
    axs[2].plot(2*np.pi*np.arange(angular_res)/angular_res, [minfo.numpy()/np.log(2) for minfo in mutual_infos], 'g')
    offset = 0.0 if max_ind/angular_res % 0.25 > 0.1 else 0.25
    axs[2].plot([2*np.pi*(max_ind/angular_res % 0.25 + offset), 2*np.pi*(max_ind/angular_res % 0.25 + offset)], [0, mutual_info/np.log(2)], 'g--')

    axs[2].text(2*np.pi*(max_ind/angular_res % 0.25 + offset)+0.1, max(0.05, mutual_info/np.log(2)-0.3), '$S(\\mathbf{f})=' + str(round(float(mutual_info/np.log(2)), 4)) + '$', color='green')
    ...
    plt.savefig('reducibility_' + dataset_name + '_' + str(round(float(P), 4)) + '_' + str(round(float(mutual_info.numpy()), 4)) + '.pdf', bbox_inches='tight')
    plt.close()
