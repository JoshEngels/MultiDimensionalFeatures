...
for i, row in enumerate(original_grids):
    for j, grid in enumerate(row):
        if grid_labels[i][j] in ("one hot $\\alpha$+$\\beta$", "$\\alpha+1=\\beta$"):
            size = 3
        elif "one hot" in grid_labels[i][j] or grid_labels[i][j] == "circle $\\alpha-\\beta$":
            size = 4
        else:
            size = 5
        ...
        ax.text(pixelsize*((mod+1)*i+mod/2), pixelsize*((mod+vertical_space)*j+mod+second_label_height), r2_labels[i][j], horizontalalignment='center', size=5)
    ax.text(pixelsize*((mod+1)*i+mod/2), pixelsize*(-1), "layer " + str(i+17), horizontalalignment='center', size=6)
plt.axis('off')
